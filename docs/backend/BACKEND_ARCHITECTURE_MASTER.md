...
| **P2 Finance ledger** | rate_cards, ledger, invoice line items, expenses, payouts, budgets; revenue/cost/margin services; invoice generation job. | Revenue, cost, margin, AR computed from real data; invoice generated from approved time; `/admin/revenue` + client `/payments` + dev `/earnings` live. |
