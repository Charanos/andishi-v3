...
**Change (done July 2, 2026, ADR-0011)**: `/api/contact` (covers both the `/start-project` wizard's build track and the `hire` track) and `/api/general-inquiry` (the `/contact` page's short form) **persist a `lead`** (keeping the Resend notification) instead of their previous inconsistent mechanisms.
...
blog_posts ✅       id, slug unique, title, category, excerpt, cover_image, author_name/role/avatar_url,
                              date_published, date_modified, read_time, featured bool, body jsonb,
                              status(enum:'published'|'draft'|'archived'), author_user_id?
case_studies        (backed by projects.isPublic today) → kept on projects table; revisions + editorial status
                              deferred. Public /work AND homepage showcase both read published projects ordered by featuredOrder.
services_content ✅ id, slug(ServiceType) unique, title, description, icon, timeline, glow(enum), group(enum), tagline,
                              image_url?, scope, engagement_options jsonb, faq jsonb, stack_highlights jsonb, order int, published bool
skill_domains ✅    id, slug unique, label, eyebrow, h1, subheadline, technologies jsonb, use_cases jsonb,
                              differentiators jsonb, faq jsonb, order int, published bool
...
content_revisions ✅ id, content_type(enum), content_id, snapshot jsonb, editor_user_id?, created_at  -- version history
```
...
**Status (July 2, 2026)**: CMS module fully built. `testimonials` ✅ and `blog_posts` ✅ implemented earlier (`src/db/schema/{testimonials,blog}.ts`, `src/lib/services/cms/{testimonials,blog}.ts`) - column names mirror the frontend's existing `Testimonial`/`BlogPost` interfaces closely for a low-friction future swap. Public `GET /api/testimonials` (`?featured=true`)/`GET /api/blog` (`?category=`)/`GET /api/blog/[slug]`, staff `?all=true` variants, `POST`, `PATCH/DELETE` gated by `cms.testimonial.write`/`cms.blog.write` (blog's staff list additionally requires `cms.blog.read` to view drafts). `services_content`, `skill_domains`, `faqs`, `content_revisions` ✅ implemented this pass (`src/db/schema/cms.ts`, `src/lib/services/cms/{services,skill-domains,faqs,revisions}.ts`) - migration `0009_messy_penance.sql`. Public `GET /api/services-content` (`?all=true` for staff)/`GET /api/services-content/[slug]`, `GET /api/skill-domains`/`GET /api/skill-domains/[slug]`, `GET /api/faqs` (`?section=`), staff `POST`/`PATCH .../entries/[id]`/`DELETE .../entries/[id]` (faqs use `PATCH/DELETE /api/faqs/[id]` directly - no public single-item route to collide with) gated by `cms.service.write`/`cms.skill_domain.write`/`cms.faq.write` respectively. `content_revisions` is a generic version-history table (`src/lib/services/cms/revisions.ts`): every create/update across all five CMS content types appends a post-write snapshot, queryable via `GET /api/content-revisions?contentType=&contentId=` gated by that content type's own write permission. `skill_domains.eyebrow` was added beyond the master doc's abbreviated column list - it's a real rendered field on every domain page (`src/data/skills.ts`'s `SkillDomainData`), added for the same near-zero-reshape-swap reasoning as every other CMS table. Verified live: 23-check end-to-end run (create/publish/staff-vs-public visibility/deny-path/revision-count/cross-type isolation) against real Neon data, all passing. Admin CMS dashboard UI for all five content types remains a follow-up, not done in this pass.

### 6.7 Marketing ✅ implemented July 2, 2026

```
campaigns ✅        id, name, channel(enum), status(enum:'draft'|'active'|'paused'|'completed'|'archived'),
                              start_date?, end_date?, budget_cents, utm jsonb, owner_user_id?
newsletter_subscribers ✅ id, email unique, status(enum:'subscribed'|'unsubscribed'), source?, subscribed_at, unsubscribed_at?
campaign_metrics ✅ id, campaign_id, date, impressions, clicks, conversions, spend_cents  -- ingestion target (GA4/manual)
```

`src/db/schema/marketing.ts`, migration `0010_white_norman_osborn.sql`. Full service layer (`src/lib/services/marketing/{campaigns,newsletter,campaign-metrics}.ts`), gated by the existing `marketing.campaign.read/write`/`marketing.newsletter.read/write` permission keys (already in the catalog since P0, never wired to tables until now) - `campaign_metrics` reuses its parent campaign's permissions rather than getting its own key, same sub-resource pattern as `crm/deal-activities.ts`. `campaign_metrics` has a unique `(campaign_id, date)` index so re-ingesting a GA4 export upserts instead of duplicating (`recordCampaignMetric` uses `onConflictDoUpdate`). Campaigns can only be hard-deleted while still `draft` - anything that progressed past that has real `campaign_metrics` history that would cascade-delete, so it must be archived (status transition) instead, same reasoning as CRM's no-delete-on-deals rule. `newsletter_subscribers` exposes public, unauthenticated, rate-limited (`POST /api/newsletter/subscribe`, 5/hour/IP) idempotent subscribe (re-subscribing an unsubscribed email flips it back rather than erroring) and `POST /api/newsletter/unsubscribe` - same public-write shape as `crm/leads.ts`'s `recordIntakeLead` (no `CallerContext`, no audit - there's no staff actor to attribute). Staff routes: `GET/POST /api/campaigns`, `GET/PATCH/DELETE /api/campaigns/[id]`, `GET/POST /api/campaigns/[id]/metrics`, `GET /api/campaigns/[id]/metrics/summary` (aggregate ROI view), `GET /api/newsletter/subscribers`. Verified live: 16-check e2e run (deny-path, draft-only-delete guard, metric upsert-not-duplicate, subscribe/unsubscribe/re-subscribe idempotency) against real Neon data. **Known, explicitly deferred gap**: the homepage newsletter form (`blog-faq-newsletter.tsx`'s `FaqNewsletterSection`) still only does `setSubscribed(true)` locally with no fetch call - wiring it to `POST /api/newsletter/subscribe` is a trivial follow-up, intentionally not done in this pass since the user was concurrently mid-edit on that exact file (its blog section, not the newsletter form) and this backend phase avoids touching files under active frontend WIP. Admin campaign dashboard UI is also a follow-up, same pattern as every other module.

### 6.8 Support & Notifications ✅ implemented July 2, 2026

```
support_cases ✅    id, subject, status(enum:'open'|'waiting'|'escalated'|'resolved'), priority(enum:'low'|'normal'|'urgent'),
                              source(enum:'client'|'developer'|'internal'), topic(enum), requester_user_id, organization_id?,
                              project_id?, assignee_user_id?, sla_minutes?, next_action?, resolution_note?, created_at
...
notification_prefs ✅ user_id, channel(enum:'email'|'in_app'|'sms'), event_type, enabled  -- composite PK
```

`src/db/schema/support.ts`, migration `0011_watery_speed_demon.sql`. Columns reconcile the master doc's minimal spec with the richer client-side mock already built (`admin-support-page.tsx`'s `SupportCase` type - added `topic`, `projectId`, `slaMinutes`, `nextAction`, `resolutionNote` beyond the doc's literal list, same frontend-mirroring precedent as every other module). Full service layer (`src/lib/services/support/{cases,messages,notifications}.ts`), gated by the existing `support.case.read/write/assign`/`support.message.write` permission keys. Access model: staff with `support.case.read` see every case; a client/developer sees and manages only cases where they're `requesterUserId` (self-service, no permission needed - mirrors `talent/availability.ts`'s self-or-staff pattern). Opening a case requires its first message in the same call - a case with zero messages isn't useful. `support_messages.internal` is enforced server-side, not just UI-hidden: a non-staff request is always forced to `internal: false` regardless of what it asks for, so a client can never post (or see) an internal note. A new non-internal message on a `resolved` case reopens it automatically. `notifications` has no public/staff creation API at all - `createNotification()` is an internal-only helper other services call directly (wired so far: support case assignment and non-internal replies); there's no "create a notification for any user" surface, since arbitrary cross-user notification creation would itself be a real abuse vector. `notification_prefs` upserts on its composite `(user_id, channel, event_type)` key. Both notifications and prefs are strictly self-scoped - not even staff can read another user's. Verified live: 22-check e2e run (cross-user access denial on cases and notifications, internal-note visibility split, forced-non-internal enforcement, auto-reopen, notification targeting, pref upsert) against real Neon data.

**Scope note**: this pass covers ticket-style support (`support_cases`) as specified in the master doc. A broader **internal project messaging** system (a running chat thread per project/placement between admin, assigned developer, and client - distinct from a support ticket) is a related but separate feature, tracked as its own build (see the session's July 2 dashboard-audit follow-up note below Part 12).
...
| **P3 CRM/Sales** ✅ | leads (persist all intake forms), deals, proposals, activities; source attribution. | Done July 2, 2026 - see [ADR-0011](adr/ADR-0011-crm-sales.md). `src/db/schema/crm.ts` (leads, deals, proposals, deal_activities; `leads.convertedToBriefId` extends the master doc's minimal spec for a traceable lead→brief link) - migration `0006_colorful_karen_page.sql`. `/api/contact` and `/api/general-inquiry` now both call a single `recordIntakeLead()` write path instead of their previous inconsistent mechanisms (guest-account-only vs. activity-event-only) - fixes the exact gap flagged in the July 2 client-intake audit. Full service layer (`src/lib/services/crm/*`): `recordIntakeLead` (dedupes re-submissions into an existing open lead, never reopens a won/lost one), `guest-accounts.ts` (shared find-or-create org/user helper, now used by both `/api/contact` and `convertLeadToBrief` instead of being duplicated), leads CRUD + `convertLeadToBrief` (the lead→brief handoff from ADR-0007), deals CRUD with stage transitions (lost requires a reason; no delete - the permission catalog deliberately has no `crm.deal.delete`, a deal that falls through is marked lost, not erased), proposals (draft→sent→accepted/rejected, sending advances an early-stage deal but never regresses one already further along, accepting auto-wins the deal), deal activity log. 11-step live end-to-end run against real Neon data plus a direct HTTP smoke test of both intake routes, all passing - no dedicated Vitest unit tests, since this module is CRUD + branching rather than the derived-computation logic P1/P2 extracted into pure functions. Pipeline board UI in admin is a follow-up, not done in this pass (same pattern as every prior backend phase). |
| **P4 CMS** ✅ | blog_posts, services_content, skill_domains, faqs, content_revisions; case-study editorial stays on `projects` (see §6.6 consolidation note). | Done July 2, 2026. `src/db/schema/cms.ts` (servicesContent, skillDomains, faqs, contentRevisions) - migration `0009_messy_penance.sql`, joins the earlier `blog_posts`/`testimonials` tables to complete the module. Full service layer (`src/lib/services/cms/{services,skill-domains,faqs,revisions}.ts`): public unauthenticated reads of published-only rows (by slug for services/skill-domains, by `?section=` for faqs), staff CRUD gated by `cms.service.write`/`cms.skill_domain.write`/`cms.faq.write`, and a generic `content_revisions` append-on-write history table shared across all five CMS content types. 23-check live end-to-end run (create/publish/staff-vs-public visibility/deny-path/revision-count/cross-type isolation) against real Neon data, all passing. `content_authors` intentionally not built - `blog_posts` already ships flattened author columns. Admin CMS dashboard UI is a follow-up, not done in this pass (same pattern as every prior backend phase). |
| **P5 Careers/Talent Supply** ✅ | job_openings, applications, application_events, talent_pool; public `/careers` + apply; recruiter queue. | Done July 1, 2026 - see §6.5/[ADR-0006](adr/ADR-0006-careers-talent-supply.md). `talent_pool_entries` deferred (no concrete UI need yet). |
| **P6 Talent Ops depth** ✅ | skills taxonomy, engineer_skills, vetting_stages, availability_windows. | Done July 2, 2026 - see §6.4. Vetting workflow rolls a per-stage decision trail up into `engineers.vettingStatus`; availability windows feed capacity planning. |
