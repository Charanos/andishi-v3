...
## CURRENT IMPLEMENTATION UPDATE - June 9, 2026

The current hero system uses the city artwork as a restrained backdrop and keeps the proof strip minimal rather than decorative-heavy.

- `public/hero-dark.svg` and `public/hero-light.svg` remain the theme-swapped city line-art sources.
- `public/hero.png` is available as the generated/bitmap hero artwork fallback for visual QA and future art direction.
- `src/components/sections/hero-section.tsx` owns the visible hero composition.
- The "Vetted talent coverage" strip should remain a slim pill/rail: one label, compact skill chips, and a restrained metric cluster.
- Do not expand the strip into a bulky card or nested proof dashboard. It should support the hero, not become the hero.
- Hero and landing titles now use `.title-serif`; supporting copy, chips, buttons, and proof metadata remain in Outfit/mono as appropriate.
