...
### B.0 May 21, 2026 Dashboard Kickoff Addendum

The public marketing/auth pages are now considered the design foundation for the internal workspaces. The next dashboard implementation phase should use `docs/DASHBOARD_MASTER_IMPLEMENTATION.md` as the primary execution authority. Use `docs/DASHBOARD_IMPLEMENTATION_PLAYBOOK.md` only as the compact companion checklist to that master document.

Current status:

- Public/marketing/auth pages are implemented.
- `/admin/*`, `/dashboard/*`, and `/dev/*` are scaffolded under `src/app/(app)/`.
- Shared dashboard primitives exist in `src/components/dashboard/`.
- Role navigation exists in `src/data/dashboard.ts`.
- Auth, RBAC, persistence, real workflows, and integrations are not implemented yet.

Immediate implementation target:

Follow `docs/DASHBOARD_MASTER_IMPLEMENTATION.md` Part 10 for full ordering. Immediate target:
...
2. Implement login credential validation and session creation.
3. Add role-aware redirects:
   - `admin -> /admin`
   - `client -> /dashboard`
   - `developer -> /dev`
4. Add server-side route guards for each route root.
5. Upgrade `AppShell`, `RoleSidebar`, and `DashboardTopNav` before deep page work.

Width/layout rule inherited from the landing pages:

- outer page or section shell owns responsive gutters: `px-5 sm:px-8 lg:px-10`
- direct inner content container owns the max width: `mx-auto w-full max-w-[92rem]`
- narrow body copy, forms, drawers, and article-like content may keep smaller inner `max-w-*` constraints.
...
- Wrong-role access redirects to the user's correct root.
- Unauthenticated app-route access redirects to `/login?next=<encoded-path>`.
- Disabled users are denied before route content renders.
...
- Add mobile drawer state and connect the hamburger button.
- Add account menu and sign out.
- Add notification popover.
- Add role badges/counts where useful.
- Keep top nav and page content aligned to the same `max-w-[92rem]` rhythm.
...
**Current implementation**: Dedicated `AdminPipelinePage` replacing the generic shared admin workspace scaffold.
...
- Stage cards showing client, tier, vertical, assigned engineers, SLA age, total age, monthly value, priority, status, and next action.
- Search plus stage, status, and priority filters.
- Selected item command rail with inspect, move forward, schedule intro, risk toggle, priority toggle, and archive actions.
- Observability panels for stage distribution, weekly velocity, active monthly value, and health mix.
- Create pipeline item modal, intro scheduling modal, detail drawer with stage history, archive confirmation, and data matrix table.

**Legacy target**: Kanban board (same as overview but expanded)
...
## PART C: HISTORICAL EXECUTION ORDER

The public-page portion of this sequence has been completed. For dashboard work, follow `docs/DASHBOARD_MASTER_IMPLEMENTATION.md` Part 10 instead of this historical ordering.
...
Current verification instruction for this project phase: run `npx tsc --noEmit` by default. Run lint, production build, Lighthouse, or browser QA only when explicitly requested or when the implementation task specifically requires that level of verification.

1. `npx tsc --noEmit` - resolve all type errors.
2. Replace all `<img>` with `next/image` in touched active components.
3. Visual QA pass at 375px, 768px, 1280px, and 1440px - both light and dark mode when requested for implementation work.
4. Run lint, Lighthouse, or production build only when explicitly requested.
...
_End of historical spec. Public-page decisions remain useful context. Dashboard execution is now governed by `docs/DASHBOARD_MASTER_IMPLEMENTATION.md`._
