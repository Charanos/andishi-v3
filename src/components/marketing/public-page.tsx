    ...
    <section className="px-5 pb-12 pt-32 sm:px-8 lg:px-10 lg:pb-16 lg:pt-40">
      <div className="mx-auto grid max-w-[92rem] gap-8 border-b border-[var(--glass-border)] pb-10 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,30rem)] lg:items-end lg:pb-14">
        <div>
          ...
          <h1 className="title-serif max-w-[20ch] text-[clamp(2.95rem,6.8vw,5.1rem)] font-normal leading-[0.94] tracking-tight text-[var(--on-surface)]">
            {title}
          </h1>
        </div>
...
export function SectionBlock({
  ...
  return (
    <section className={cn("px-5 py-14 sm:px-8 lg:px-10 lg:py-20", className)}>
      <div className="mx-auto max-w-[92rem]">
        <div className="mb-8 max-w-3xl lg:mb-10">
          {eyebrow && (
            <p className="label-caps mb-4 text-[var(--secondary)]">{eyebrow}</p>
          )}
          <h2 className="title-serif max-w-[20ch] text-[clamp(2.25rem,5vw,3.85rem)] font-normal leading-[0.96] tracking-tight text-[var(--on-surface)]">
            {title}
          </h2>
          {body && (
            <p className="body-md my-8 max-w-2xl text-[var(--on-surface-dim)]">
              {body}
            </p>
          )}
        </div>
