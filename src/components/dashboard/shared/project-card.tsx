  ...
  return (
    <article className="rounded-[1.15rem] border border-[var(--glass-border)] bg-[var(--glass-bg)] p-5 backdrop-blur-2xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-[1.05rem] font-medium text-[var(--on-surface)]">
            {project.name}
          </h3>
        ...
        <StatusBadge
          ...
          tone={project.status === "Active" ? "active" : "pending"}
        />
      </div>
      <p className="mt-4 text-[0.94rem] leading-relaxed text-[var(--on-surface-dim)]">
        {project.description}
      </p>
      {project.sourceBrief && (
        <p className="mt-3 rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg)] px-3 py-2 text-[0.8rem] leading-relaxed text-[var(--on-surface-dim)]">
          Source brief:{" "}
