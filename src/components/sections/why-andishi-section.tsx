      ...
      <div className="relative z-[1] mx-auto max-w-[84rem]">
        <div className="mb-14 flex flex-col items-center text-center">
          ...
          <p className="body-md mt-6 max-w-2xl text-[var(--on-surface-dim)]">
            How we compare against the traditional options when you need to ship a complex digital product.
          </p>
        </div>
        ...
        <div className="hidden md:block relative z-10 max-w-6xl mx-auto mt-16">
          <div className="grid grid-cols-[minmax(200px,1.5fr)_1fr_1fr_1.15fr] gap-x-2">
            ...
            <div className="pb-6 pl-6 flex items-end border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]">
              <span className="label-caps text-[color-mix(in_srgb,var(--on-surface-dim)_70%,transparent)] tracking-[0.1em]">Evaluation Criteria</span>
            </div>
            <div className="pb-6 text-center flex items-end justify-center border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]">
              <span className="label-caps text-[color-mix(in_srgb,var(--on-surface-dim)_70%,transparent)] tracking-[0.1em]">Freelancer</span>
            </div>
            <div className="pb-6 text-center flex items-end justify-center border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]">
              <span className="label-caps text-[color-mix(in_srgb,var(--on-surface-dim)_70%,transparent)] tracking-[0.1em]">Large Agency</span>
            </div>
            <div className="pb-6 pt-4 pl-10 flex flex-col justify-end relative">
              ...
              <span className="label-caps text-[var(--on-surface)] font-bold text-[0.85rem] tracking-[0.1em] mb-1">Andishi</span>
            </div>
            ...
            {comparisonRows.map((row, index) => {
              ...
              return (
                <React.Fragment key={row[0]}>
                  {/* Criteria */}
                  <div className={`py-5 pl-6 flex items-center ${!isLast ? 'border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]' : ''}`}>
                    <span className="font-medium text-[0.98rem] text-[var(--on-surface)]">{row[0]}</span>
                  </div>
                  {/* Freelancer */}
                  <div className={`py-5 px-4 flex items-center justify-center text-center ${!isLast ? 'border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]' : ''}`}>
                    <span className="text-[0.92rem] text-[var(--on-surface-dim)] opacity-70">
                      {row[1] === "No" ? <IconX size={16} stroke={2} className="text-[var(--outline)]" /> : row[1]}
                    </span>
                  </div>
                  {/* Agency */}
                  <div className={`py-5 px-4 flex items-center justify-center text-center ${!isLast ? 'border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]' : ''}`}>
                    <span className="text-[0.92rem] text-[var(--on-surface-dim)] opacity-70">
                      {row[2] === "No" ? <IconX size={16} stroke={2} className="text-[var(--outline)]" /> : row[2]}
                    </span>
                  </div>
                  {/* Andishi (Aligned left with Checkmarks) */}
                  <div className="py-5 pl-10 pr-5 relative flex items-center justify-start">
                    <div className={`absolute inset-x-0 inset-y-0 border-x border-[color-mix(in_srgb,var(--on-surface)_8%,transparent)] bg-[color-mix(in_srgb,var(--on-surface)_3%,transparent)] backdrop-blur-md -z-10 ${isLast ? 'rounded-b-[1.5rem] border-b pb-2' : 'border-b border-[color-mix(in_srgb,var(--on-surface)_6%,transparent)]'}`} />
                    <span className="text-[0.96rem] font-medium text-[var(--on-surface)] flex items-center gap-3">
                      <IconCircleCheck size={18} stroke={2} className="text-[var(--tertiary)] shrink-0" />
                      {row[3]}
                    </span>
                  </div>
                </React.Fragment>
        ...
        <div className="md:hidden mt-12 flex flex-col gap-6">
          {comparisonRows.map((row) => (
            <div key={row[0]} className="rounded-2xl border border-[var(--glass-border)] bg-[var(--surface-low)] shadow-sm overflow-hidden">
              ...
              <div className="px-5 py-5 flex flex-col gap-4.5">
                ...
                <div className="flex items-start justify-between gap-4">
                  <span className="text-[0.7rem] uppercase tracking-[0.1em] font-medium text-[var(--on-surface-dim)] opacity-60 w-1/3">Freelancer</span>
                  <span className="text-[0.9rem] text-[var(--on-surface-dim)] opacity-80 text-right w-2/3">
                    {row[1] === "No" ? <IconX size={15} stroke={2} className="ml-auto opacity-50" /> : row[1]}
                  </span>
                </div>
                
                {/* Large Agency Row */}
                <div className="flex items-start justify-between gap-4">
                  <span className="text-[0.7rem] uppercase tracking-[0.1em] font-medium text-[var(--on-surface-dim)] opacity-60 w-1/3">Agency</span>
                  <span className="text-[0.9rem] text-[var(--on-surface-dim)] opacity-80 text-right w-2/3">
                    {row[2] === "No" ? <IconX size={15} stroke={2} className="ml-auto opacity-50" /> : row[2]}
                  </span>
                </div>
                ...
                <div className="flex items-start justify-between gap-4 pt-4 border-t border-[color-mix(in_srgb,var(--tertiary)_12%,transparent)]">
                  <span className="text-[0.75rem] uppercase tracking-[0.1em] font-bold text-[var(--on-surface)] w-1/3 mt-0.5">Andishi</span>
                  <span className="text-[0.95rem] font-medium text-[var(--on-surface)] flex flex-wrap justify-end items-center gap-2 text-right w-2/3">
                    <IconCircleCheck size={16} stroke={2} className="text-[var(--tertiary)] shrink-0" />
                    {row[3]}
                  </span>
                </div>
              </div>
