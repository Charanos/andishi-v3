...
function mapApiProjectToWorkProject(apiProj: PublicProject): WorkProject {
  return {
    ...
    imageHeight: apiProj.featuredOrder !== undefined && apiProj.featuredOrder !== null ? "mid" : "short",
          ...
          <div className="relative z-[1] mx-auto flex w-full max-w-[92rem] items-start gap-0 pb-24 pt-32 lg:pt-36">
            <aside className="sticky top-28 hidden max-h-[calc(100svh-8rem)] w-60 shrink-0 flex-col justify-between self-start overflow-y-auto border-r border-[var(--glass-border)] pr-5 xl:flex">
                    ...
                    {serviceFilters.map((filter) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-left text-[0.82rem] font-medium transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--primary)_50%,transparent)]"
                          style={{
                    ...
                    {verticalFilters.map((filter) => {
                      ...
                      return (
                        <button
                          key={filter.value}
                          type="button"
                          onClick={() => setActiveVertical(filter.value)}
                          className="flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-left text-[0.82rem] font-medium transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--primary)_50%,transparent)]"
                          style={{
                            backgroundColor: isActive
                              ? "color-mix(in srgb, var(--primary) 10%, transparent)"
              ...
              <div className="space-y-3 mt-6">
                {[
                  ["32+", "products shipped successfully"],
                  ["48h", "to scoping brief proposal"],
                  ...
                  >
                    ...
                    <p className="mt-2 text-[0.72rem] leading-snug text-[var(--on-surface-dim)]">
                      {label}
                    </p>
                  </div>
                ))}
            ...
            <div className="min-w-0 flex-1 xl:pl-10">
              <header className="mb-10 border-b border-[var(--glass-border)] pb-8 md:mb-12 md:grid md:grid-cols-[1fr_auto] md:items-end md:gap-10">
                ...
                >
                  <p className="font-mono text-[clamp(3rem,7vw,5rem)] leading-none tracking-tight text-[color-mix(in_srgb,var(--on-surface)_28%,transparent)] dark:text-[color-mix(in_srgb,var(--on-surface)_14%,transparent)]">
                    {formatIndex(filteredProjects.length - 1)}
                  </p>
              ...
              <div className="mb-8 flex flex-col gap-4 lg:mb-10 lg:flex-row lg:items-center xl:hidden">
                <div className="flex items-center gap-2 text-[0.76rem] font-medium uppercase tracking-[0.12em] text-[var(--on-surface-dim)]">
              ...
              <motion.div
                key={`${activeService}-${activeVertical}`}
                initial={{ opacity: 0 }}
              ...
              <section className="relative mt-20 overflow-hidden rounded-[1.5rem] border border-[var(--glass-border)] bg-[color-mix(in_srgb,var(--surface)_42%,transparent)] px-6 py-12 text-center shadow-[0_24px_80px_color-mix(in_srgb,var(--bg-deep)_24%,transparent)] backdrop-blur-2xl sm:px-10 lg:mt-24 lg:px-16 lg:py-16">
                ...
                <div className="relative z-[1] mx-auto max-w-2xl">
                  ...
                  <p className="body-md mx-auto my-8 max-w-lg text-[var(--on-surface-dim)]">
                    Tell us what you are building. We will provide a direct fixed-price
                    brief and timeline to get your product launched.
                  </p>
        ...
        <>
          ...
          >
            ...
            <div className="px-5 pb-10 pt-2 sm:px-8 sm:pb-12">
              ...
              <h2
                id="case-study-title"
                className="title-serif max-w-2xl text-[clamp(2.12rem,4.2vw,2.85rem)] font-normal leading-[1.06] tracking-tight text-[var(--on-surface)]"
              >
                {project.title}
              </h2>
              <p className="body-md my-8 max-w-3xl text-[var(--on-surface-dim)]">
                {project.description}
              </p>
