...
export function ContactPageExperience() {
  ...
  const submitBrief = (event: FormEvent<HTMLFormElement>) => {
    ...
    const body = [
      ...
      "",
      `Product Type: ${selectedServices.join(", ")}`,
  ...
  return (
    <main className="relative isolate overflow-hidden bg-[var(--bg)] px-5 sm:px-8 lg:px-10">
      ...
      <section className="relative z-[1] mx-auto grid min-h-[calc(100svh-5rem)] w-full max-w-[92rem] gap-0 pb-24 pt-32 lg:grid-cols-[1.02fr_0.98fr] lg:pt-36">
        <motion.div
        ...
        >
          ...
          <h1 className="title-serif max-w-[17ch] text-[clamp(3.12rem,7vw,5.05rem)] font-normal leading-[0.94] tracking-tight text-[var(--on-surface)]">
            Tell us about your project.
          </h1>
          <p className="body-md mt-7 max-w-xl text-[var(--on-surface-dim)]">
            Three steps. Two minutes. Outline your goals, timeline, and tech stack so we can provide a detailed scope, timeline, and transparent pricing brief.
          </p>
              ...
              {submittedRef ? (
                ...
                <>
                  ...
                  {step === 1 && (
                    ...
                    >
                      <ChoiceGroup
                        label="What kind of product are we building?"
                        options={services}
                      ...
                      <label className="grid gap-2">
                        ...
                        <textarea
                          ...
                          placeholder="Outline the core features, your timeline constraints, tech stack preferences, and any integrations (like payments or APIs) you need."
            ...
            {!submittedRef && (
              <div className="mt-8 flex items-center justify-between gap-4 border-t border-[color-mix(in_srgb,var(--on-surface)_16%,transparent)] pt-6 dark:border-[var(--glass-border)]">
                ...
                {step < 2 ? (
                  ...
                  >
                    {step === 0 ? "Project details" : "Timing and scale"}
        ...
        >
          <div className="lg:sticky lg:top-28">
            ...
            <div
              className={`my-8 rounded-[1rem] border p-5 backdrop-blur-xl ${controlSurfaceClass}`}
            >
            ...
            <div className="my-8 flex flex-wrap justify-center gap-x-3 gap-y-1 text-center">
              {[
                "No pitch",
...
function SuccessState({ refCode }: { refCode: string }) {
  return (
    ...
    >
      ...
      <h2 className="title-serif mt-6 text-[clamp(1.85rem,3.4vw,2.35rem)] font-normal leading-tight tracking-tight text-[var(--on-surface)]">
      ...
      <p className="body-md mt-4 max-w-md text-[var(--on-surface-dim)]">
        ...
        there and the Andishi team will respond with a scope draft within 24 hours.
      </p>
      <p className="my-8 rounded-lg border border-[color-mix(in_srgb,var(--secondary)_20%,transparent)] bg-[color-mix(in_srgb,var(--secondary)_10%,transparent)] px-4 py-2 font-mono text-[0.74rem] tracking-tight text-[var(--secondary)]">
        ref: {refCode}
      </p>
    </motion.div>
  );
...
function NairobiPanel() {
  return (
    <div className="relative overflow-hidden rounded-[1.35rem] border border-[color-mix(in_srgb,var(--on-surface)_18%,transparent)] bg-[color-mix(in_srgb,var(--surface)_90%,var(--bg)_10%)] p-5 shadow-[0_24px_70px_color-mix(in_srgb,var(--bg-deep)_14%,transparent)] backdrop-blur-2xl dark:border-[var(--glass-border)] dark:bg-[color-mix(in_srgb,var(--surface)_40%,transparent)] dark:shadow-[0_24px_70px_color-mix(in_srgb,var(--bg-deep)_22%,transparent)]">
      ...
      <div className="relative my-8 grid gap-3 sm:grid-cols-3">
        {[
          [IconClock, "24hrs", "response target"],
          [IconCalendarTime, "30min", "scoping call"],
          [IconCheck, "3-5 days", "scoping brief"],
