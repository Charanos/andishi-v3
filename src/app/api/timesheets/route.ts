...
import { listTimesheets, logTime } from "@/lib/services/delivery/timesheets";
...
export async function GET() {
  ...
  try {
    const result = await listTimesheets({ session, requestId });
...
export async function POST(req: NextRequest) {
  ...
  try {
    const timesheet = await logTime({ session, requestId, actorIp: getClientIp(req) }, parsed.data);
