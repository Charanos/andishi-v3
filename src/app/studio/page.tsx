...
const services = [
  [
    "Web apps",
    "Operational portals, dashboards, booking systems, and workflow tools.",
    IconDeviceDesktop,
  ],
  [
    "E-commerce",
    "M-Pesa-ready storefronts, inventory workflows, and order dashboards.",
    IconShoppingCart,
  ],
  [
    "Conversion sites",
    "Fast, credible websites for businesses that need trust and qualified leads.",
    IconWorldWww,
  ],
  [
    "Digital systems",
    "Internal systems that replace spreadsheets, manual status checks, and disconnected tools.",
    IconPlugConnected,
  ],
] as const;
    ...
    <>
      <PublicPageShell>
        ...
        <SectionBlock
          ...
          title="Focused builds with product discipline."
        >
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {services.map(([title, body, Icon]) => (
              <GlassPanel key={title}>
                ...
                <h2 className="my-8 text-[1.05rem] font-medium text-[var(--on-surface)]">
                  {title}
                </h2>
        ...
        <SectionBlock eyebrow="How studio works" title="Brief, sprint, ship.">
          <div className="grid gap-4 lg:grid-cols-3">
            {[
              [
                "Brief",
                "We clarify the business workflow, customer journey, technical constraints, and the result that matters.",
              ],
              [
                "Sprint",
                "A small senior team builds in focused loops with visible progress and practical decisions.",
              ],
              [
                "Ship",
                "We hand over a live product with deployment, admin context, and the next improvement path.",
              ],
              ...
              <GlassPanel key={title}>
                ...
                <h2 className="my-8 text-[1.12rem] font-medium text-[var(--on-surface)]">
                  {title}
                </h2>
        ...
        <SectionBlock
          eyebrow="Featured studio work"
          title="Proof from shipped systems."
        >
          <div className="grid gap-4 md:grid-cols-3">
            {featured.map((project) => (
              <GlassPanel key={project.id}>
                ...
                <h2 className="mt-4 text-[1.15rem] font-medium leading-tight text-[var(--on-surface)]">
                  {project.title}
                </h2>
                <p className="mt-4 text-[0.93rem] leading-relaxed text-[var(--on-surface-dim)]">
                  {project.description}
                </p>
                <div className="my-8">
                  <LinkButton
